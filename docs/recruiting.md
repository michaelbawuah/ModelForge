...
The final validation layer includes a hosted release verifier that checks the
public HTTPS/security/auth/runtime surface before running the same deployment
proof against a real cloud instance. Until that hosted workflow passes, the
repository deliberately claims validated AWS architecture and hosted-proof
readiness rather than a completed public deployment.
...
- 168 Python tests passed; 3 skipped in the standard Python CI job.
